...
def predict_ensemble(classifier_strings, training_vectors, vector_classes, test_vectors):
    ...
    for classifier_string in classifier_strings:
        ...
        for i, p in enumerate(prediction_probabilities):
            # p[i] is the probability that instance i belongs to class specified by classifier_string
...
def predict_ensemble_precalculated(scores):
    """
    scores is a list of length N, where N is theamount of training instances.
    each entry fwervetv
    """
    for i in scores:
        combined = (np.sum(scores[i].values(), axis=0) / float(len(scores[i])))[0]
...
class Classifiers():
    """
    Lets us be dynamic from the command line on which classifer we want to use
    """
    classifiers = {
